...
def run_multi_queries(n_runs=100):
    ...
    os.makedirs("/data/ep924610/project_nb/paper_code/llm_zero_shot/raw", exist_ok=True)
    ...
    for filename in os.listdir("/data/ep924610/project_nb/paper_code/llm_zero_shot/raw"):
    ...
    if openai_todo:
        ...
        def process_openai(run_id):
            ...
            if result:
                ...
                with open(f"/data/ep924610/project_nb/paper_code/llm_zero_shot/raw/openai_run_{result['run_id']}.json", "w") as f:
    ...
    if claude_todo:
        ...
        def process_claude(run_id):
            ...
            if result:
                # Save individual result
                with open(f"/data/ep924610/project_nb/paper_code/llm_zero_shot/raw/claude_run_{result['run_id']}.json", "w") as f:
                    json.dump(result, f, indent=2)
    ...
    for run_id in range(n_runs):
        ...
        openai_file = f"/data/ep924610/project_nb/paper_code/llm_zero_shot/raw/openai_run_{run_id}.json"
        ...
        claude_file = f"/data/ep924610/project_nb/paper_code/llm_zero_shot/raw/claude_run_{run_id}.json"
    ...
    with open("/data/ep924610/project_nb/paper_code/llm_zero_shot/openai_all_results.json", "w") as f:
        json.dump(all_openai_results, f, indent=2)
    
    with open("/data/ep924610/project_nb/paper_code/llm_zero_shot/claude_all_results.json", "w") as f:
...
def analyze_results(openai_results, claude_results):
    ...
    all_weights.to_csv("/data/ep924610/project_nb/paper_code/llm_zero_shot/extracted_weights.csv", index=False)
    ...
    summary.to_csv("/data/ep924610/project_nb/paper_code/llm_zero_shot/weight_summary_stats.csv")
    ...
    top_combos.to_csv("/data/ep924610/project_nb/paper_code/llm_zero_shot/top_weight_combinations.csv", index=False)
...
def create_visualizations(all_weights, summary):
    ...
    os.makedirs("/data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations", exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig("/data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations/weight_distributions.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig("/data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations/average_weights_heatmap.png", dpi=300)
    ...
    radar_fig.savefig("/data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations/weight_radar_chart.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig("/data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations/top_weight_combinations.png", dpi=300)
    
    logger.info("Visualizations created in /data/ep924610/project_nb/paper_code/llm_zero_shot/visualizations/")
...
def generate_summary_report(all_weights, summary, top_combos):
    ...
    # Save the report
    with open("/data/ep924610/project_nb/paper_code/llm_zero_shot/weight_analysis_report.md", "w") as f:
        f.write("\n".join(report))
    
    logger.info("Summary report generated: /data/ep924610/project_nb/paper_code/llm_zero_shot/weight_analysis_report.md")
...
def load_existing_results():
    ...
    try:
        # Load OpenAI results
        if os.path.exists("results_zero_shot/openai_all_results.json"):
            with open("results_zero_shot/openai_all_results.json", "r") as f:
        ...
        # Load Claude results
        if os.path.exists("results_zero_shot/claude_all_results.json"):
            with open("results_zero_shot/claude_all_results.json", "r") as f:
...
def get_gemini_progress():
    ...
    try:
        if os.path.exists("results_zero_shot/gemini_all_results.json"):
            with open("results_zero_shot/gemini_all_results.json", "r") as f:
...
def run_gemini_queries(total_runs=1000, gene_lists=None, batch_size=25, max_threads=1):
    ...
    os.makedirs("results_zero_shot", exist_ok=True)
    os.makedirs("results_zero_shot/raw", exist_ok=True)
    ...
    for batch_start in range(0, len(runs_to_do), batch_size):
        ...
        for run_id in batch_runs:
            ...
            if result:
                ...
                with open(f"results_zero_shot/raw/gemini_run_{result['run_id']}.json", "w") as f:
                    json.dump(result, f, indent=2)
                pbar.update(1)
            else:
        ...
        with open("results_zero_shot/gemini_all_results.json", "w") as f:
...
def analyze_all_models():
    ...
    # Save extracted weights
    all_weights.to_csv("results_zero_shot/all_models_weights.csv", index=False)
    logger.info(f"Saved extracted weights from all models. Total: {len(all_weights)} rows")
    
    # Create summary statistics
    summary = all_weights.groupby("model")[weight_features].agg(['mean', 'median', 'std', 'min', 'max'])
    summary.to_csv("results_zero_shot/all_models_summary_stats.csv")
    ...
    top_combos.to_csv("results_zero_shot/all_models_top_combinations.csv", index=False)
...
def create_three_model_visualizations(all_weights, summary):
    ...
    os.makedirs("results_zero_shot/visualizations", exist_ok=True)
    ...
    plt.tight_layout()
    plt.savefig("results_zero_shot/visualizations/three_model_weight_distributions.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig("results_zero_shot/visualizations/three_model_average_weights_heatmap.png", dpi=300)
    ...
    radar_fig.savefig("results_zero_shot/visualizations/three_model_radar_chart.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig("results_zero_shot/visualizations/three_model_relative_importance.png", dpi=300)
    ...
    for feature in weight_features:
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f"results_zero_shot/visualizations/violin_{feature}.png", dpi=300)
    ...
    for feature in ["Heatmap", "HPA", "Clinical_trials"]:  # Top 3 most important features
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f"results_zero_shot/visualizations/density_{feature}.png", dpi=300)
...
def generate_comparative_report(all_weights, summary, top_combos):
    ...
    # Save the report
    with open("results_zero_shot/three_model_analysis_report.md", "w") as f:
        f.write("\n".join(report))
    
    logger.info("Comparative report generated: results_zero_shot/three_model_analysis_report.md")
...
if __name__ == "__main__":
    ...
    data_dir = "/data/ep924610/project_nb/paper_code/llm_prompt_prep_zero_shot"  # Path to your CSV files
    ...
    try:
        gene_lists = None
        if os.path.exists("/data/ep924610/project_nb/paper_code/llm_prompt_prep_zero_shot"):
            gene_lists = load_gene_lists_from_files("/data/ep924610/project_nb/paper_code/llm_prompt_prep_zero_shot", top_n=100)
    except Exception as e:
    ...
    logger.info("Analysis complete! Check the 'results_zero_shot' directory for outputs.")
