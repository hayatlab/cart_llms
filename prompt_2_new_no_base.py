...
# Keep SVG text editable and avoid embedded glyph paths
mpl.rcParams['svg.fonttype'] = 'none'


# Optional but commonly requested for Adobe workflows
mpl.rcParams['pdf.fonttype'] = 42      # TrueType fonts
mpl.rcParams['ps.fonttype'] = 42

# Choose fonts Illustrator is likely to have (falls back to DejaVu Sans if missing server-side)
...
# Output directory
OUTPUT_DIR = "/data/ep924610/project_nb/paper_code/new_prompt2_no_base"
os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(f"{OUTPUT_DIR}/raw", exist_ok=True)
os.makedirs(f"{OUTPUT_DIR}/visualizations_revised", exist_ok=True)
...
openai_limiter = RateLimiter(tokens_per_min=10000, max_parallel_requests=2)
claude_limiter = RateLimiter(tokens_per_min=90000, max_parallel_requests=2)
gemini_limiter = RateLimiter(tokens_per_min=10000, max_parallel_requests=2)
...
[GENES LIST: [ERBB3, RNF43, MC1R, PMEL, LZTS1, NRG2, NRG4, IGF1R, PTCH1, GPNMB, HPS4, SORT1, MAK, BLNK, TNFRSF14, LTK, ROBO1, ADIPOR2, CDH1, TNFSF13B, TCHP, PKD2, LAT, EPOR, EPS8, PIGF, CDH3, DDR2, ABCB4, NKTR, CHRNA5, PDCD1LG2, CD226, MLLT10, BMPR2, ERBB2, F2RL1, IFNAR2, NEO1, CTNS, HTR2B, TGFBR1, BCL2L12, FHIT, EPHA2, FUT2, APC, HCAR2, ADAM12, TRPM7, DST, TNK2, FCGR2B, GPER1, NPIPA1, GPR55, PPL, IL6R, AXIN2, RASGRP1, PDE4D, HAS3, SLC22A1, RAF1, BRAF, PMP22, ITGA6, PTK2, INSR, WLS, STIL, RAD51B, MYO10, NF1, IL1RAP, P2RX7, CNIH3, LEPR, GRK4, ABCA7, CADM1, TRPV2, SLC47A1, SLC16A1, SLC39A4, SLC38A6, WDPCP, PTCH2, LAPTM4B, TNFSF12, CRCP, MR1, CTTN, ZNRF3, NCAM1, GPC3, MME, CATSPER2, GNAQ, TEC]
...
def extract_gene_data(response_text, model_name=None, run_id=None):
    ...
    gene_list = ['ERBB3', 'RNF43', 'MC1R', 'PMEL', 'LZTS1', 'NRG2', 'NRG4', 'IGF1R', 'PTCH1', 'GPNMB', 'HPS4', 'SORT1', 'MAK', 'BLNK', 'TNFRSF14', 'LTK', 'ROBO1', 'ADIPOR2', 'CDH1', 'TNFSF13B', 'TCHP', 'PKD2', 'LAT', 'EPOR', 'EPS8', 'PIGF', 'CDH3', 'DDR2', 'ABCB4', 'NKTR', 'CHRNA5', 'PDCD1LG2', 'CD226', 'MLLT10', 'BMPR2', 'ERBB2', 'F2RL1', 'IFNAR2', 'NEO1', 'CTNS', 'HTR2B', 'TGFBR1', 'BCL2L12', 'FHIT', 'EPHA2', 'FUT2', 'APC', 'HCAR2', 'ADAM12', 'TRPM7', 'DST', 'TNK2', 'FCGR2B', 'GPER1', 'NPIPA1', 'GPR55', 'PPL', 'IL6R', 'AXIN2', 'RASGRP1', 'PDE4D', 'HAS3', 'SLC22A1', 'RAF1', 'BRAF', 'PMP22', 'ITGA6', 'PTK2', 'INSR', 'WLS', 'STIL', 'RAD51B', 'MYO10', 'NF1', 'IL1RAP', 'P2RX7', 'CNIH3', 'LEPR', 'GRK4', 'ABCA7', 'CADM1', 'TRPV2', 'SLC47A1', 'SLC16A1', 'SLC39A4', 'SLC38A6', 'WDPCP', 'PTCH2', 'LAPTM4B', 'TNFSF12', 'CRCP', 'MR1', 'CTTN', 'ZNRF3', 'NCAM1', 'GPC3', 'MME', 'CATSPER2', 'GNAQ', 'TEC']
...
def create_visualizations(gene_rank_stats, avg_ranks, reason_counts, reason_data_detailed=None):
    """Create focused visualizations_revised for gene analysis with requested specifications"""
    ...
    plt.savefig(f"{OUTPUT_DIR}/visualizations_revised/top_genes_by_reason.svg", dpi=300)
    plt.close()
    
    logger.info(f"Saved gene analysis visualizations_revised to {OUTPUT_DIR}/visualizations_revised/")
def create_reason_cooccurrence_plots(gene_rank_stats, reason_data):
    ...
    os.makedirs(f"{OUTPUT_DIR}/visualizations_revised/cooccurrence", exist_ok=True)
        ...
        for gene in model_top_genes:
            try:
                ...
                if gene_reasons.empty:
                    ...
                    plt.tight_layout()
                    plt.savefig(f"{OUTPUT_DIR}/visualizations_revised/cooccurrence/{model}_{gene}.svg", dpi=300)
                    plt.close()
                    continue
    ...
    try:
        ...
        with PdfPages(f"{OUTPUT_DIR}/visualizations_revised/all_cooccurrence_plots.pdf") as pdf:
            for model in models:
                model_top_genes = gene_rank_stats[gene_rank_stats['model'] == model].groupby('gene')['count'].sum().sort_values(ascending=False).head(10).index.tolist()
                
                for gene in model_top_genes:
                    try:
                        img = plt.imread(f"{OUTPUT_DIR}/visualizations_revised/cooccurrence/{model}_{gene}.svg")
    ...
    logger.info(f"Created co-occurrence plots for top genes in {OUTPUT_DIR}/visualizations_revised/cooccurrence/")
    return True
...
if __name__ == "__main__":
    ...
    # Create visualizations_revised with both aggregated counts and detailed reason data
    ...
    # Create better co-occurrence visualizations_revised using the detailed reason data
    ...
    def create_upset_plots(reason_data):
        ...
        # Ensure output directory exists
        os.makedirs(f"{OUTPUT_DIR}/visualizations_revised/upset_plots", exist_ok=True)
        ...
        for model in models:
            ...
            for gene in top_genes:
                try:
                    ...
                    # Save the plot
                    plt.savefig(f"{OUTPUT_DIR}/visualizations_revised/upset_plots/{model}_{gene}_upset.svg", dpi=300, bbox_inches='tight')
                    plt.close()
                    ...
                    if filtered.empty:
                        ...
                        plt.tight_layout()
                        plt.savefig(f"{OUTPUT_DIR}/visualizations_revised/xy_plots/{model}_{gene}_run_reason_cumulative.svg", dpi=300)
                        plt.close()
        ...
        try:
            ...
            with PdfPages(f"{OUTPUT_DIR}/visualizations_revised/all_upset_plots.pdf") as pdf:
                for model in models:
                    ...
                    for gene in top_genes:
                        try:
                            filepath = f"{OUTPUT_DIR}/visualizations_revised/upset_plots/{model}_{gene}_upset.svg"
        ...
        print(f"Created upset plots in {OUTPUT_DIR}/visualizations_revised/upset_plots/")
        return True
    ...
    def plot_combined_upset_top_genes(reason_df, output_dir, top_n=10):
        ...
        for gene in top_genes:
            ...
            outfile = f"{output_dir}/visualizations_revised/upset_plots/ALLMODELS_{gene}_upset_stacked.svg"
    ...
    def plot_gene_nominations_by_model_unique_runs(reason_data, output_dir, top_n=30):
        ...
        plot_path = f"{output_dir}/visualizations_revised/gene_nominations_by_model_unique_runs.svg"
    ...
    def plot_top_genes_stacked_barplot(reason_data, output_dir, top_n=10):
        ...
        outdir = os.path.join(output_dir, "visualizations_revised")
    ...
    def plot_simple_alluvial(
        ...
        html_path = f"{output_dir}/visualizations_revised/model_gene_reason_alluvial_simple.html"
        fig.write_html(html_path)
        print(f"Interactive alluvial plot saved: {html_path}")

        try:
            png_path = f"{output_dir}/visualizations_revised/model_gene_reason_alluvial_simple.svg"
    ...
    def make_gene_nominations_csv(reason_data, output_path):
        ...
        return out
    make_gene_nominations_csv(all_reason_data, "/data/ep924610/project_nb/paper_code/new_prompt2_no_base/gene_nominations_summary.csv")
    def plot_reason_stackedbar_error_top(reason_data, output_dir):
        ...
        plt.tight_layout()
        plot_path = f"{output_dir}/visualizations_revised/reason_stackedbar_error_top.svg"
        plt.savefig(plot_path, dpi=300)
        plt.close()
